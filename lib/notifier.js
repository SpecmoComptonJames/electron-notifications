...
class Notifier {
  ...
  getWorkAreaSize() {
    return (electron.screen) ? electron.screen.getPrimaryDisplay().workAreaSize : electron.remote.screen.getPrimaryDisplay().workAreaSize
  }
  ...
  notify (title, data) {
    ...
    const size = this.getWorkAreaSize();
    var verticalSpace = 0;
